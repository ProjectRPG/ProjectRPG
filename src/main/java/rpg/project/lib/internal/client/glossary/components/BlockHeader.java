...
public class BlockHeader extends ReactiveWidget {
    private final Identifier id;
    private final String name;
    public BlockHeader(Block block) {
        super(0,0,50,20);
        Minecraft mc = Minecraft.getInstance();
        this.id = RegistryUtil.getId(mc.player.registryAccess(), block);
        this.name = block.getName().toString();
        addChild(new ItemDisplayWidget(mc, 0,0,16,16, block.getName(), block.asItem().getDefaultInstance(), false, true)
    ...
    public boolean applyFilter(Filter filter) {
        return !(filter.matchesTextFilter(id.toString())
                ...
                || !filter.matchesObject(ObjectType.BLOCK);
    }
